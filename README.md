...
- **走势图**：给持仓和绿灯候选自动画 价格+均线+成交量 图，内嵌进 Markdown 报告里看。
- **报告自动归档**：每天存一份带日期的 Markdown 报告，历史可回看。
...
| `今日报告.md` | 脚本生成的技术面报告（含内嵌走势图，不入库）|
| `今日报告（深度研究）.md` | `/分析` 生成的联网深度报告（不入库）|
| `charts/` | 走势图 PNG（被报告引用，不入库）|
| `reports/` | 两种报告的按日期归档（不入库）|

> **关于早期脚本**：`analyze_storage.py`、`refresh_status.py` 等保留未删。它们**不是被新工具完全取代**，而是做的事不同——老脚本有**配对交易(多空 pair) z-score**、并覆盖**三星/海力士/铠侠等亚洲内存股**（带汇率折算），这些 `daily_check.py` 没做（本工具只做美股）。日常的"加仓/卖出"判断已被 `daily_check.py` 覆盖，所以平时用不到老脚本。
